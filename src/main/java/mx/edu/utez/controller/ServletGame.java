    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        switch(action){
            case "create":
                ...
                if(flag){
                    map.put("message", "Se ha registrado exitosamente.");
                } else {
                    map.put("message", "No se registró, intenta más tarde.");
                }
                break;
            case "update":
                beanCategory.setIdCategory(Integer.parseInt(request.getParameter("idCategory")));
                ...
                if(flag1){
                    map.put("message", "Se actualizo, exitosamente.");
                } else {
                    map.put("message", "No se actualizó, intenta más tarde.");
                }
                break;
            case "delete":
                if(new DaoGames().delete(Integer.parseInt(request.getParameter("idGame")))){
                    request.setAttribute("message", "Se elimino exitosamente");
                } else {
                    CONSOLE.error("No se eliminó, intente más tarde.");
                }
                break;
            default:
